...
| **SPOT WND** | **数字だけ入れると自動で `/` が入る**（`24030` → `240/30`、`040135` → `040/135`） |
| **RMG** | `284.1` / `2841` / `284100` どの書き方でも可。PLAN FUEL との差を自動表示 |

**EEP**（ETOPS ENTRY POINT）の地点名は紫で表示し、枠も紫にして見つけやすくしてある。
...
- **ETO を BackSpace で消すと `0000` に張り付いて直せなくなる不具合を修正。**
  ...
  分かるようにした。
- **EEP** の地点名を紫（`--eep`）にし、ストリップの枠も紫に。
- **SPOT WND** は数字だけ入力すれば `/` が入る（先頭 3 桁が風向）。
