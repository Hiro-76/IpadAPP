/* =======================================================================
   SWING OVERLAY — service worker
   scope は golf/ 配下のみ。ルートの NAVLOG アプリとは独立して動作する。
   VERSION を上げれば全ファイルを取り直す。
   ======================================================================= */
const VERSION = 'swingoverlay-v1.0.0';
const CACHE   = VERSION;
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    await Promise.all(names.filter(n => n !== CACHE && n.startsWith('swingoverlay-'))
